...
for (var i = 0; i < phrases.length; i++) {
    console.log(`phrases[${i}].length ${phrases[i].length}`);
    if(phrases[i].length > max_len){
        console.log(`phrases[${i}].length = ${phrases[i].length} > ${max_len}`);
        max_len = phrases[i].length;
        console.log(`max_len = ${max_len}`);
        index_max = i;
    }
}

DIV_HTML.innerHTML = 'La phrase la plus longue est : "' + phrases[index_max] + '" et elle fait ' + max_len + ' caractère(s) !';
